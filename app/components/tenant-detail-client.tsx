    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      {metrics && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
          {[
            { label: 'Ventas totales',     value: String(metrics.totalSales),                                     color: 'var(--cyan)' },
            { label: 'Ventas este mes',    value: String(metrics.salesThisMonth),                                 color: 'var(--success)' },
            { label: 'Ingresos este mes',  value: `$${Number(metrics.revenueThisMonth).toLocaleString('es-CL')}`, color: 'var(--warning)' },
            { label: 'Productos cargados', value: String(metrics.totalProducts),                                  color: 'var(--text)' },
          ].map((s, i) => (
            <div key={i} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px' }}>
              ...
              <p style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 700, color: s.color }}>{s.value}</p>
            </div>
          ))}
        </div>
      )}
            ...
            {users.map(u => (
              <div key={u.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: 'var(--bg2)', borderRadius: 8, border: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button onClick={() => { setResetUser(u); setShowReset(true) }} style={{
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <button onClick={() => setShowPayForm(true)} style={{
            padding: '7px 14px', background: 'var(--cyan)', color: 'var(--bg)',
        ...
        {payments.length === 0 ? (
          <p style={{ fontSize: 13, color: 'var(--muted)', textAlign: 'center', padding: '24px' }}>
              ...
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: 'var(--bg2)', borderRadius: 8, border: '1px solid var(--border)' }}>
                <div>
                  ...
                  <p style={{ fontSize: 11, color: 'var(--muted)' }}>
                    {p.method}{p.period && ` · ${p.period}`}{p.note && ` · ${p.note}`}
                  </p>
                </div>
                <p style={{ fontSize: 12, color: 'var(--muted)' }}>
      ...
      {showPayForm && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '28px', width: '100%', maxWidth: 420 }}>
            ...
            <form onSubmit={handlePayment} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <div>
                  ...
                  <select style={inputStyle} value={payForm.method}
                ...
                <div>
                  ...
                  <input style={inputStyle} placeholder="Ej: Julio 2026" value={payForm.period}
      ...
      {showReset && resetUser && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '28px', width: '100%', maxWidth: 380 }}>
            ...
            <form onSubmit={handleResetPassword} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <input required type="password" autoComplete="new-password" style={inputStyle}
                  value={resetPass} onChange={e => setResetPass(e.target.value)}
                  placeholder="Mínimo 8 caracteres" />
              </div>
              <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                <button type="button" onClick={() => { setShowReset(false); setResetUser(null); setResetPass('') }}
                  style={{ flex: 1, padding: '10px', background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--muted)', fontSize: 13, cursor: 'pointer' }}>
                  Cancelar
                </button>
                <button type="submit" disabled={resetLoading} style={{ flex: 1, padding: '10px', background: 'var(--cyan)', color: 'var(--bg)', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
