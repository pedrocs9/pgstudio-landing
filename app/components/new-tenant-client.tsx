...
export default function NewTenantClient() {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div>
              ...
              <select style={inputStyle} value={form.businessType}
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            ...
            <input required style={inputStyle} value={form.adminName}
              onChange={e => setForm({ ...form, adminName: e.target.value })} />
          </div>
          <div>
            <label style={labelStyle}>Email *</label>
            <input required type="email" style={inputStyle} value={form.adminEmail}
              onChange={e => setForm({ ...form, adminEmail: e.target.value })} />
          </div>
          <div style={{ gridColumn: '1 / -1' }}>
            <label style={labelStyle}>Contraseña inicial *</label>
            <input required type="password" autoComplete="new-password" style={inputStyle} value={form.password}
      ...
      {/* Módulos */}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px' }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 600, color: 'var(--text)', marginBottom: 4 }}>
          Módulos contratados
        </h2>
          ...
          {MODULES.map(m => (
            <button key={m.key} type="button" onClick={() => toggleModule(m.key)}
              style={{
                ...
                background: selectedModules.includes(m.key) ? 'rgba(14,165,233,0.08)' : 'transparent',
                cursor: m.required ? 'not-allowed' : 'pointer',
                opacity: m.required ? 0.7 : 1,
              }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 13, fontWeight: 500, color: selectedModules.includes(m.key) ? 'var(--cyan)' : 'var(--text)' }}>
                  {m.label}
                </span>
              ...
              {m.required && <p style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>Incluido siempre</p>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
          <div>
            ...
            <input required type="number" style={inputStyle} value={form.basePrice}
              onChange={e => setForm({ ...form, basePrice: e.target.value })} />
          </div>
          <div style={{ display: 'flex', alignItems: 'flex-end' }}>
            <div style={{ background: 'var(--bg2)', borderRadius: 10, padding: '10px 16px', width: '100%' }}>
              <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 4 }}>Total mensual</p>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 700, color: 'var(--cyan)' }}>
                ${totalPrice} USD
              </p>
            </div>
          </div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button type="button" onClick={() => router.push('/admin/vexor')} style={{
